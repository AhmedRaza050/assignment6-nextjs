...
const Nav = ({openNav}: Props) => {
  ...
  return (
    <div className={`fixed ${navBg ? "bg-gray-800": ""} h-[10vh] z-[100] w-full transition-all duration-200 bg-black
    `}>
      <div className="flex items-center h-full justify-between w-[95%] sm:w-[90%] xl:w-[80%] mx-auto">


        <div className="flex items-center space-x-2">
          <div className="md:w-10 md:h-10 w-7 h-7 rounded-full bg-black text-white flex items-center justify-center flex-col">
            <FaHouse />
          </div>
          ...
          {navLinks.map((navlink) => {
            return (
              <Link key={navlink.id} href={navlink.url}>
                <p className="font-medium hover:text-red-400">
                  {navlink.label}
                </p>
              </Link>
            );
